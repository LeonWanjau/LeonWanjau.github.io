...
export default class RenderLineText {
    constructor(canvas, sceneBackground) {
        //canvas and renderer
        this.canvas = canvas
        this.renderer = new THREE.WebGLRenderer({ canvas: this.canvas, antialias: true })
        this.renderer.setSize(this.canvas.clientWidth, this.canvas.clientHeight)
        this.renderer.setClearColor(new THREE.Color(sceneBackground), 1)
        ...
        this.scene.background = new THREE.Color(sceneBackground) //Previous 0xf0f0f0
    ...
    createLineMesh(shapes, xMid) {
        const color = 0x006699
        const threeColor = new THREE.Color(0x006699)
        const strokeWidth = 3
        ...
        const style = SVGLoader.getStrokeStyle(strokeWidth, threeColor.getStyle())
        ...
        for (let i = 0; i < shapes.length; i++) {
            ...
            let lineMesh = new THREE.Mesh(geometry, matDark)
